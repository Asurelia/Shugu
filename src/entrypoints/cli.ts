...
async function main(): Promise<void> {
  ...
  try {
    const { services, systemPrompt, needsHatchCeremony, resumedMessages, resumedWorkContext, resumedWorkContextData } = await bootstrap(cliArgs);

    if (cliArgs.prompt) {
      await runSingleQuery(services, cliArgs.prompt, systemPrompt);
    } else {
      await runREPL(services, systemPrompt, needsHatchCeremony, resumedMessages, resumedWorkContext, resumedWorkContextData);
    }
  } catch (error) {
