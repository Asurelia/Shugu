...
export const helpCommand: Command = {
  ...
  async execute(args, ctx) {
    ...
    ctx.info('  /thinking        Toggle expanded thinking display');
    ctx.info('  /expand          Dump full transcript');
    ctx.info('  /rewind [N]      Undo last N turns (default: 1)');
    ctx.info('  /resume [id]     Resume a previous session');
    ctx.info('');
    ctx.info('  Project');
    ctx.info('  /init            Initialize project (SHUGU.md + .pcc/)');
    ctx.info('  /doctor          Run diagnostic health checks');
    ctx.info('  /status          Show git status and project info');
    ctx.info('  /diff            Show git diff with colors');
    ctx.info('  /commit [msg]    Generate commit message and commit');
    ctx.info('  /review          Review recent code changes');
    ctx.info('  /export [file]   Export conversation to file');
    ctx.info('');
    ctx.info('  Model & Tools');
    ctx.info('  /model [name]    Show or change active model');
    ctx.info('  /fast            Toggle fast mode (M2.5)');
    ...
    ctx.info('  /memory save     Save a note');
    ctx.info('');
    ctx.info('  Companion');
